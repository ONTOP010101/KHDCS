        ...
        <div class="inventory-form-scroll" style="flex:1;overflow-y:auto;min-width:0">
          <div class="inventory-form-grid">
            ...
            <div class="inventory-form-row inventory-form-row-6">
              ...
              <div class="inventory-form-field">
                <label class="inventory-form-label">出库时间</label>
                <input class="inventory-form-input" style="width:380px" :value="currentRecord?.stockOutTime || ''" readonly />
              </div>
              <div class="inventory-form-field">
          ...
          <div class="form-modal-body">
            <div class="fm-row fm-row-2">
              <div class="fm-field">
                ...
                <input class="fm-input fm-input-auto" :value="formData.outboundCode" readonly placeholder="自动生成" />
              </div>
              <div class="fm-field">
    ...
    <div v-if="showBatchAddModal" class="modal-overlay" style="background:transparent;backdrop-filter:none" @click.self="showBatchAddModal = false">
      <div class="modal-dialog" style="max-width:1500px;width:96vw;max-height:65vh;display:flex;flex-direction:column">
        <div class="modal-header">
          <strong style="font-size:30px">批量添加出库</strong>
...
import {
  Search, Plus, Pencil, Trash2, Save, X, Send,
...
const doAddMaterial = async () => {
  try {
    ...
    const res = await api(`/outbound/${codeName}/items`, {
      method: 'POST',
...
const doBatchAdd = async () => {
  ...
  try {
    ...
    const res = await api(`/outbound/${codeName}/items/batch`, {
      method: 'POST',
...
const columns = [
  ...
  { field: 'stockOutTime', title: '出库时间', minWidth: 160, sortable: true },
...
const { fullKey: gridStorageKey, saveToBackend: saveGridPrefs, ready: prefReady } = useGridPrefSync(gridRef, 'outbound-detail', columns)
...
const saveOutbound = async () => {
  ...
  try {
    if (formMode.value === 'add') {
      const res = await api('/outbound', { method: 'POST', body: JSON.stringify(payload) })
      if (res && res.code === 200) {
      ...
      const res = await api(`/outbound/${id}`, { method: 'PUT', body: JSON.stringify(payload) })
      if (res && res.code === 200) {
...
const deleteCurrent = async () => {
  ...
  try {
    const res = await api(`/outbound/${currentRecord.value.id}`, { method: 'DELETE' })
    if (res && res.code === 200) {
...
const batchSubmit = () => {
  ...
  showConfirmDialog(msg, async () => {
    ...
    try {
      ...
      const res = await api('/outbound/submit', { method: 'PUT', body: JSON.stringify(ids) })
      if (res && res.code === 200) {
        checkedRows.value = []
...
const loadData = async () => {
  try {
    ...
    const res = await api(`/outbound?${params.join('&')}`)
